...
**Progress note (as of latest continuation)**: Phase 1 video+audio+progress+basic-offline completed on feature/phase1-video-audio-offline (see commits: video UX tall fixed player + dedicated captions, progress save/resume on page+video end, Phase1 complete feat). Then added pages json cache + full VideoCache (Android + iOS FS impls) + use cases + UI triggers + local path preference in players so the fixed video player works offline (download MP4s for anahita etc stories). Updated ROADMAP. (DB on iOS still optional until bootstrap wires driver + iosVideoCacheModule). Next per roadmap: full offline bundles, lullabies, rate/lang for audio, auth, etc.
